...
/*                        $responseData = json_encode(array(
                    ...
                    if($result['code']=='0') { //Update Data 
                        ...
                        foreach($rows as $key=>$value) {
                            if(trim($value['elem_name'])!='' && trim($value['elem_src_name'])!='') {
...
/*                    $responseData[] = json_encode(array(
                    ...
                    //Get data for demo
                    $this->db_chgth = $this->load->database('chgth',true);
                    $rows = $this->db_chgth->query("select * from stat_thai_population where year_of_stat='{$year_of_stat}'")->result_array();
                    $responseData_decode = $rows;
                    $code = '-1'; $message = 'Not Found Data';
                    if(count($rows)>0) {
                        $code = '0'; $message = '';
                        //Update Center DB
                        $this->common_model->delete_where('stat_thai_population','year_of_stat',$year_of_stat);
                        $this->common_model->insert_batch('stat_thai_population',$responseData_decode);

                        //Update Data Warehouse
                        $this->db_gateway->insert_batch('dtawh_stat_thai_population',$responseData_decode);
                    }
                    //End Get data for demo

                    //$responseData = array();
                    //foreach ($responseData_decode as $key => $value) {
                    //       $responseData[$key] = $value;
                    //       $responseData[$key]['code'] = '0';
                    //       $responseData[$key]['message'] = '';
                    //}

                    $responseData = $responseData_decode;
                    $request['result'] = array('code'=>$code,'message'=>$message,'rows'=>count($responseData_decode));
                    //$request['result'] = $responseData;
...
/*                    $responseData[] = json_encode(array(
                    ...
                    //Update Center DB
                    $tmp = $this->common_model->custom_query("select * from pers_qol_indicators where addr_home_no='{$addr_home_no}' AND addr_sub_district='{$addr_sub_district}'");
                    if(count($tmp)>0) {
                        $this->db->where(array('addr_home_no'=>$addr_home_no,'addr_sub_district'=>$addr_sub_district));
                        $this->db->delete('pers_qol_indicators');
                    }
                    //End Update Center DB
                    ...
                    foreach ($responseData_decode as $key => $value) {
                        ...
                        //Update Center DB
                        $elements = $this->transfer_model->getAll_elementWSV(8,'Element');
                        $data_update = array();
                        foreach($elements as $key1=>$value1) {
                            if(trim($value1['elem_name'])!='' && trim($value1['elem_src_name'])!='') {
                                $data_update[trim($value1['elem_name'])] = trim($value[trim($value1['elem_src_name'])]);
                            }
                        }
                        $this->common_model->insert('pers_qol_indicators',$data_update);
                        //End Update Center DB
                    }
                    $request['result'] = $responseData;
    ...
    public function RequestElderyJPTH_get() {
        // dtawh_pers_qol_indicators
        $rows = $this->db_gateway->query("select * from wsrv_log where wsrv_id='8'")->result_array();
        $responseData = array();
        ...
        foreach($rows as $key=>$data) {
            $tmp = unserialize($data['log_note']);
            if(count($tmp['result'])>0) {
                if(!isset($tmp['result']['code'])) {
                    foreach($tmp['result'] as $row) {
                        $tmp_arr = array();
                        foreach($elements as $key1=>$value1) {
                            if(trim($value1['elem_name'])!='' && trim($value1['elem_src_name'])!='') {
                                $tmp_arr[trim($value1['elem_name'])] = trim($row[trim($value1['elem_src_name'])]);
                            }
                        }
                        $tmp_arr['rec_update'] = date("Y-m-d H:i:s");
                        $data_insert[] = $tmp_arr;                        
                    }        
                }
            }
        ...
        //Insert Data Warehouse DB
        if(count($data_insert)>0) {
            $this->db_gateway->insert_batch('dtawh_pers_qol_indicators',$data_insert);
            $responseData = $data_insert;
        }

        if(count($rows)>0) {
            $this->response($responseData,200);
        }else {
            $this->response(array('code'=>'-1','message'=>'Not Found Data'),200);
        }
    }
...
/*                    $responseData[] = json_encode(array(
                    ...
                    foreach ($responseData_decode as $key => $value) {
                        ...
                        //Update Data
                        if($value['code']=='0') { //Center DB
                            unset($value['code']);unset($value['message']);
                            $tmp = $this->transfer_model->getOnce_wsrv_info(9);
                            $value['rec_source'] = $tmp['owner_org'];
                            $value['rec_update'] = date("Y-m-d H:i:s");
                            $code = $value['code']; $message = $value['message'];

                            //Update Center DB
                            $tmp = rowArray($this->common_model->custom_query("select * from edoe_job_vacancy where posi_id=".$value['posi_id']));
                            if(isset($tmp['posi_id'])) {
                                $this->common_model->update('edoe_job_vacancy',$value,array('posi_id'=>$tmp['posi_id']));
                            }else {
                                $this->common_model->insert('edoe_job_vacancy',$value);
                            }

                            //Update Data Warehouse
                            $this->db_gateway->insert('dtawh_edoe_job_vacancy',$value);                           
                        }
                        //End Update Data

                    }
                    //$request['result'] = $responseData;
                    $request['result'] = array('code'=>$code,'message'=>$message,'rows'=>count($responseData));
    ...
    public function RequestOlderEmploymentRegistration_post(){
        header('Access-Control-Allow-Origin: *');

        $eldery_pid = $this->post('eldery_pid');
...
/*                    $responseData[] = json_encode(array(
                    ...
                    foreach ($responseData_decode as $key => $value) {
                           ...
                           //Update Center DB
                           if($responseData[$key]['code']=='0') {
                                $tmp = $responseData[$key];
                                unset($tmp['code']);unset($tmp['message']);unset($tmp['pers_id']);unset($tmp['pid']);
                                
                                $tmp1 = rowArray($this->common_model->custom_query("select * from pers_info where pid='{$responseData[$key]['pid']}' AND delete_datetime IS NULL order by pers_id"));
                                if(isset($tmp1['pers_id'])) {
                                    //$this->common_model->delete_where('edoe_older_emp_reg','pers_id',$tmp1['pers_id']);
                                    $tmp2 = rowArray($this->common_model->custom_query("select ereg_id from edoe_older_emp_reg where pers_id={$tmp1['pers_id']} order by ereg_id"));

                                    $tmp['pers_id'] = $tmp1['pers_id'];
                                    $owner_org = $this->transfer_model->getOnce_wsrv_info(10);
                                    $tmp['rec_source'] = $owner_org['owner_org'];
                                    $tmp['rec_update'] = date("Y-m-d H:i:s");

                                    if(!isset($tmp2['ereg_id'])) {
                                        $this->common_model->insert('edoe_older_emp_reg',$tmp);
                                    }else {
                                        $this->common_model->update('edoe_older_emp_reg',$tmp,array('ereg_id'=>$tmp2['ereg_id']));
                                    }
                                }
                           }
                           //End Update Center DB
                    ...
                    foreach($responseData as $key=>$data) {
                        $responseData[$key]['date_of_reg_th'] = formatDateThai($responseData[$key]['date_of_reg']);
                    }
...
*/
                    ...
                    foreach ($responseData_decode as $key => $value) {
                        ...
                        //Update Center DB
                        $tmp = rowArray($this->common_model->custom_query("select pers_id from pers_info where pid='{$TargetPID}' AND delete_datetime IS NULL order by pers_id"));

                        if(isset($responseData_decode['code'])) {
                            if(isset($tmp['pers_id'])) {
                                //$this->common_model->delete_where('pers_elderly_foundation','pers_id',$tmp['pers_id']);
                                $tmp2 = rowArray($this->common_model->custom_query("select efnd_id from pers_elderly_foundation where pers_id={$tmp['pers_id']}"));

                                if(!isset($tmp2['efnd_id'])) {
                                    $this->common_model->insert('pers_elderly_foundation',array('pers_id'=>$tmp['pers_id'],'loan_history'=>'ไม่มีประวัติ'));
                                }else {
                                    $this->common_model->update('pers_elderly_foundation',array('pers_id'=>$tmp['pers_id'],'loan_history'=>'ไม่มีประวัติ'),array('efnd_id'=>$tmp2['efnd_id']));       
                                }
                            }
                            $responseData = array('pid'=>$TargetPID,'loan_history'=>'ไม่มีประวัติ','apprv_province'=>'','year_of_contract'=>'','contract_status'=>'','code'=>$responseData_decode['code'],'message'=>$responseData_decode['message']);
                            break;
                        }else {
                            $code = isset($value['code'])?$value['code']:0; $message = isset($value['message'])?$value['message']:'';
                            $pid = $value['idnumbercard'];
                            unset($value['code']);unset($value['message']);
                            $value['loan_history'] = 'มีประวัติ';

                            if(strlen($value['budgetyear'])==2) {
                                $value['budgetyear'] = '25'.$value['budgetyear'];
                            }

                            if(strlen($value['year_contract'])==2) {
                                $value['year_of_contract'] = '25'.$budgetyear;
                                unset($value['year_contract']);
                            }else {
                                $value['year_of_contract'] = '';
                                unset($value['year_contract']);
                            }
                            
                            if($value['status']=='01') {
                                $value['contract_status'] = 'ยังมีสัญญา';
                                unset($value['status']);
                            }else {
                                $value['contract_status'] = 'ปิดสัญญาแล้ว';
                                unset($value['status']);
                            }
                            $value['province_code'] = $value['province_code'].'000000';
                            
                            unset($value['result_status']);unset($value['idnumbercard']);unset($value['province_approv']);unset($value['budgetyear']);unset($value['credit_line']);

                            if(isset($tmp['pers_id'])) {
                                //$this->common_model->delete_where('pers_elderly_foundation','pers_id',$tmp['pers_id']);
                                $tmp2 = rowArray($this->common_model->custom_query("select efnd_id from pers_elderly_foundation where pers_id={$tmp['pers_id']}"));
                                $value['pers_id'] = $tmp['pers_id'];
                                
                                if(!isset($tmp2['efnd_id'])) {
                                    $this->common_model->insert('pers_elderly_foundation',$value);
                                }else {
                                    $this->common_model->update('pers_elderly_foundation',$value,array('efnd_id'=>$tmp2['efnd_id']));       
                                }
                            }
                            $value['pid'] = $pid;
                            $value['code'] = $code;
...
/*    public function RequestElderyFoundation_get() {
        // dtawh_pers_qol_indicators
        $rows = $this->db_gateway->query("select * from wsrv_log where wsrv_id='13'")->result_array();
        $responseData = array();
        
        $elements = $this->transfer_model->getAll_elementWSV(13,'Element');
        $data_insert = array();
        ...
        foreach($rows as $key=>$data) {
            ...
            if(count($tmp['result'])>0) {
                if(!isset($tmp['result']['code'])) {
                    //Update Data warehouse DB
                    $tmp1 = rowArray($this->common_model->custom_query("select pers_id from pers_info where pid='{$tmp['result']['pid']}' AND delete_datetime IS NULL order by pers_id"));

                    if(isset($tmp1['pers_id'])) {
                        $data_insert[] = array('pers_id'=>$tmp1['pers_id'],'loan_history'=>'ไม่มีประวัติ','rec_update'=>date('Y-m-d H:i:s')));
                    }
                    //Update Data warehouse DB
        ...
        //Insert Data Warehouse DB
        if(count($data_insert)>0) {
            $this->db_gateway->insert_batch('dtawh_pers_elderly_foundation',$data_insert);
            $responseData = $data_insert;
        }
        ...
        if($Username=='' || $Password=='' || $OfficerPID=='' || $TargetPID=='') {
            ...
            if($this->transfer_model->chk_wsrvPermission($authen_user['authen_id'],14)) {
                try {                     
                    ...
                    //$url = "http://www.fmop.dop.go.th/dopapi/web/borrowers/datum?user=dopuser&password=dpuser&id=3120200164001";

                    //$url = 'http://www.fmop.dop.go.th/dopapi/web/borrowers/datum/3120200164001';
                    //dieFont($url);
...
/*                        //Update Center DB
                        ...
                        if(isset($responseData_decode['code'])) {
                            if(isset($tmp['pers_id'])) {
                                //$this->common_model->delete_where('pers_elderly_foundation','pers_id',$tmp['pers_id']);
                                $tmp2 = rowArray($this->common_model->custom_query("select brrw_id from stat_thai_elderly_foundation where pers_id={$tmp['pers_id']}"));

                                if(!isset($tmp2['brrw_id'])) {
                                    $this->common_model->insert('stat_thai_elderly_foundation',array('pers_id'=>$tmp['pers_id'],'loan_history'=>'ไม่มีประวัติ'));
                                }else {
                                    $this->common_model->update('stat_thai_elderly_foundation',array('pers_id'=>$tmp['pers_id'],'loan_history'=>'ไม่มีประวัติ'),array('brrw_id'=>$tmp2['brrw_id']));       
                                }
                            }
                            $responseData = array('pid'=>$TargetPID,'year_of_req'=>'','loan_history'=>'ไม่มีประวัติ','apprv_province'=>'','year_of_contract'=>'','contract_status'=>'','code'=>$responseData_decode['code'],'message'=>$responseData_decode['message']);
                            break;
                        }else {
                            $code = isset($value['code'])?$value['code']:0; $message = isset($value['message'])?$value['message']:'';
                            $pid = $value['idnumbercard'];
                            ...
                            if(strlen($value['budgetyear'])==2) {
                                $value['budgetyear'] = '25'.$value['budgetyear'];
                                $value['year_of_req'] = $value['budgetyear'];
                            }
                            ...
                            if(isset($tmp['pers_id'])) {
                                //$this->common_model->delete_where('stat_thai_elderly_foundation','pers_id',$tmp['pers_id']);
                                $tmp2 = rowArray($this->common_model->custom_query("select efnd_id from stat_thai_elderly_foundation where pers_id={$tmp['pers_id']}"));
                                $value['pers_id'] = $tmp['pers_id'];
                                
                                if(!isset($tmp2['efnd_id'])) {
                                    $this->common_model->insert('stat_thai_elderly_foundation',$value);
                                }else {
                                    $this->common_model->update('stat_thai_elderly_foundation',$value,array('efnd_id'=>$tmp2['efnd_id']));       
                                }
                            }
                            $value['pid'] = $pid;
                            $value['code'] = $code;
    ...
    //ข้อมูลกรมบัญชีกลาง
    //ธุรกรรมไอดี เลขบัตร วันที่ร้องขอ วันที่อนุมัติ จำนวนเงิน remark org_id ชื่อสกุล อายุ วันเดือนปีเกิด
    public function RequestCGDPayment_get($UserName='3100602549624',$Password='0001', $AppID='9580007172',$OfficerPID='3100602549624',$OfficerCID='40422403150b4e9cwe',$DateRequest='2017-12-14',$TranferAmount='2000',$Remark='ค่าสงเคราห์ในการจัดการงานศพ',$OrganID='79',$TargetPID='3100602549624') {
        ...
        $request['param'] = array('AppID'=>$AppID,'OfficerPID'=>$OfficerPID,'OfficerCID'=>$OfficerCID,'DateRequest'=>$DateRequest,'TranferAmount'=>$TranferAmount,'Remark'=>$Remark,'OrganID'=>$OrganID,'TargetPID'=>$TargetPID);

        if($AppID=='' || $OfficerPID=='' || $OfficerCID=='' || $DateRequest=='' || $TranferAmount=='' || $Remark=='' || $OrganID=='' || $TargetPID=='') {
            $msg = array('message'=>'BAD_REQUEST (400)','code'=>'-1');
            $request['result'] = $msg;
            $this->transfer_model->wsrv_logSave($authen_user['authen_id'],$authen_user['pid'],$time_start,24,serialize($request),'Import','Fail'); //Save Log
            $this->response($request['result'], REST_Controller::HTTP_BAD_REQUEST);// BAD_REQUEST (400) being the HTTP response code   
        }else if(isset($authen_user['authen_id'])) {

            if($this->transfer_model->chk_wsrvPermission($authen_user['authen_id'],24)) {
                try {
                    $url = $this->setUrl1(24);
                    ...
                    $data = array(
                        ...
                        'paraAppID' => $AppID,
                        'paraOfficerPID' => $OfficerPID,
                        'paraOfficerCID' => $OfficerCID,
                        'paraTargetPID' => $TargetPID,
                        'paraDateRequest' => $DateRequest,
                        'paraTranferAmount' => $TranferAmount,
                        'paraRemark' => $Remark,
                        'paraOrganID' => $OrganID,
                    );
                    $response = Requests::post($url, $headers, json_encode($data));
                    ...
                    $this->transfer_model->wsrv_logSave($authen_user['authen_id'],$authen_user['pid'],$time_start,24,serialize($request),'Import','Success'); //Save Log

                    //echo $responseData;
                    $this->response($responseData_decode,200);
                }catch (Exception $e) {
                    $msg = array('message'=>$e->getMessage(),'code'=>'-1');
                    $request['result'] = $msg;
                    $this->transfer_model->wsrv_logSave($authen_user['authen_id'],$authen_user['pid'],$time_start,24,serialize($request),'Import','Fail'); //Save Log
            ...
            $msg = array('message'=>'Authen ID Invalid','code'=>'-1');
            $request['result'] = $msg;
            $this->transfer_model->wsrv_logSave('',$OfficerPID,$time_start,24,serialize($request),'Import','Fail'); //Save Log
                //echo json_encode($msg);
            $this->response($request['result'], REST_Controller::HTTP_NOT_FOUND);// NOT_FOUND (404) being the HTTP response code
        }
